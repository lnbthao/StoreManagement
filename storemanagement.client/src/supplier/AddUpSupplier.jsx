...
export default function AddUpSupplier() {
  ...
  const validateField = (key, val) => {
    ...
    if (key === "phone") {
      ...
      else if (!/^\d{9,11}$/.test(val.trim())) msg = "SĐT chỉ chứa số (9–11 chữ số).";
    }
    if (key === "email") {
      ...
      <form onSubmit={handleSubmit} noValidate>
        {["name", "phone", "email", "address"].map((key) => (
          <div className="mt-3" key={key}>
            <label htmlFor={`supplier-${key}`} className="d-block mb-1">
              {key === "name" ? "Tên nhà cung cấp:" : key.charAt(0).toUpperCase() + key.slice(1) + ":"}
            </label>
            <input
              id={`supplier-${key}`}
              className={`form-control ${errors[key] ? "is-invalid" : ""} mb-1`}
              placeholder={`Nhập ${key === "name" ? "tên nhà cung cấp" : key}`}
